...
function minimumInt(item: ConfigMeta): number {
  return item.key === 'login_expiry_days' ? 1 : 0
}

function maximumInt(item: ConfigMeta): number | undefined {
  return item.key === 'login_expiry_days' ? 365 : undefined
}
