...
func TestGenerateTokenWithTTL(t *testing.T) {
	ttl := 30 * 24 * time.Hour
	tok, err := GenerateTokenWithTTL(7, "remembered", "user", 1, testSecret, ttl)
	if err != nil {
	...
	if err != nil || !parsed.Valid {
		t.Fatalf("parse remembered token: %v", err)
	}
	claims := parsed.Claims.(*Claims)
	gotTTL := claims.ExpiresAt.Time.Sub(claims.IssuedAt.Time)
	if gotTTL != ttl {
		t.Fatalf("token TTL = %v, want %v", gotTTL, ttl)
	}
	if _, err := GenerateTokenWithTTL(7, "invalid", "user", 1, testSecret, 0); err == nil {
		t.Fatal("GenerateTokenWithTTL accepted a non-positive TTL")
	}
}
