...
// GenerateToken preserves the historical 7-day default for callers that do
// not need a custom session lifetime.
func GenerateToken(userID uint, username, role string, authVersion uint, secret string) (string, error) {
	return GenerateTokenWithTTL(userID, username, role, authVersion, secret, defaultTokenTTL)
}

// GenerateTokenWithTTL issues an HS256 JWT for the requested lifetime. The
...
func GenerateTokenWithTTL(userID uint, username, role string, authVersion uint, secret string, ttl time.Duration) (string, error) {
	if ttl <= 0 {
