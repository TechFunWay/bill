...
type Config struct {
	...
	// FnOSApp enables trusted identity forwarding through the fnOS unified
	// gateway. Normal TCP and Docker deployments leave this disabled.
	FnOSApp       bool
...
func Parse() Config {
	flag.IntVar(&C.Port, "port", 8907, "服务监听端口（环境变量 PORT）")
	...
	flag.StringVar(&C.GatewayPrefix, "gateway-prefix", "/app/techfunway-bill", "飞牛统一网关路径前缀（仅 -fnos-app）")
	...
	if C.DBPath == "" {
		C.DBPath = filepath.Join(C.DataDir, "db", "bill.db")
	}
	if C.LogDir == "" {
...
示例:
  bill -port 8080 -data-dir /var/lib/bill
  DATA_DIR=/app/data RATE_LIMIT=0 bill
`)
}
