...
// TestRegistryContainsBuiltins locks the eight built-in config definitions:
// seven system-scope and one user-scope (theme_mode).
func TestRegistryContainsBuiltins(t *testing.T) {
	...
	// Then: all eight built-ins are present in their scope
	for _, key := range []string{"site_title", "site_description", "allow_register", "require_login", "login_expiry_days", "log_retention_days", "jwt_secret"} {
	...
	retention, ok := findDef(system, "log_retention_days")
	if !ok || retention.Type != TypeInt || retention.Default != "30" {
		t.Fatalf("log_retention_days def wrong: %+v", retention)
	}
	if Validate(retention, "-1") == nil {
		t.Fatal("negative log retention should be rejected")
	}
	loginExpiry, ok := findDef(system, "login_expiry_days")
	if !ok || loginExpiry.Type != TypeInt || loginExpiry.Default != "30" || !loginExpiry.Public || loginExpiry.Group != "access" {
		t.Fatalf("login_expiry_days def wrong: %+v", loginExpiry)
	}
	for _, invalid := range []string{"0", "366"} {
		if Validate(loginExpiry, invalid) == nil {
			t.Fatalf("login expiry %s should be rejected", invalid)
		}
	}
}
...
func TestInitDefaultConfigsPreservesAdminEdits(t *testing.T) {
	...
	if count != 7 {
		t.Fatalf("system config rows = %d, want 7 (no duplicates)", count)
	}
}
