...
type App struct {
	...
	SetupAdmin  func(api *gin.RouterGroup, db *gorm.DB) // administrator-only route registration
...
// SetupProtectedAll wires authenticated and administrator-only app routes
// after the framework has attached the corresponding middleware. Keeping this
// separate preserves the original public Setup extension point while making
// authorization requirements explicit for modular apps.
