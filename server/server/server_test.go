...
func tokenDuration(t *testing.T, token, secret string) time.Duration {
	t.Helper()
	parsed, err := jwt.ParseWithClaims(token, &auth.Claims{}, func(t *jwt.Token) (interface{}, error) {
...
func TestLoginRememberUsesConfiguredExpiry(t *testing.T) {
	r, db, _ := setupTestRouter(t)
	registerUser(t, r, "admin", "secret123")
	if err := sysconfig.UpdateConfig(db, "login_expiry_days", "30", 0); err != nil {
		t.Fatalf("set login expiry: %v", err)
	}
	secret, err := sysconfig.GetConfig(db, "jwt_secret", 0)
	if err != nil {
	...
	login := func(remember bool) string {
		w := doJSON(r, http.MethodPost, "/api/auth/login", "", map[string]interface{}{
			"username": "admin",
			"password": "secret123",
			"remember": remember,
		})
		if w.Code != http.StatusOK {
			t.Fatalf("login remember=%v: status %d body %s", remember, w.Code, w.Body.String())
		}
		return decode(t, w)["data"].(map[string]interface{})["token"].(string)
	}

	if got := tokenDuration(t, login(false), secret); got != 24*time.Hour {
		t.Fatalf("session token TTL = %v, want 24h", got)
	}
	if got := tokenDuration(t, login(true), secret); got != 30*24*time.Hour {
		t.Fatalf("remembered token TTL = %v, want 30d", got)
	}

	public := doJSON(r, http.MethodGet, "/api/configs/public", "", nil)
	if got := decode(t, public)["data"].(map[string]interface{})["login_expiry_days"]; got != "30" {
		t.Fatalf("public login expiry = %v, want 30", got)
	}
}
...
func TestAuthenticatedAppRoutesDoNotUsePublicRateLimiter(t *testing.T) {
	baseRouter, db, dir := setupTestRouter(t)
	token := registerUser(t, baseRouter, "admin", "secret123")
	secret, err := sysconfig.GetConfig(db, "jwt_secret", 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := doJSON(r, http.MethodGet, "/api/billing/dashboard", "", nil).Code; got != http.StatusUnauthorized {
		t.Fatalf("anonymous dashboard request returned %d, want 401", got)
	}
	for i := 0; i < 2; i++ {
		if got := doJSON(r, http.MethodGet, "/api/billing/dashboard", token, nil).Code; got != http.StatusOK {
			t.Fatalf("dashboard request %d returned %d, want 200", i+1, got)
		}
	}
}
