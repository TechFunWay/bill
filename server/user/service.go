...
var (
	...
	ErrUserHasAppData   = errors.New("该用户已有账单数据，不能删除；可改为禁用账号")
)

const (
	sessionTokenTTL          = 24 * time.Hour
	defaultRememberLoginDays = 30
	maxRememberLoginDays     = 365
)

func rememberedLoginTTL(db *gorm.DB) time.Duration {
	days := defaultRememberLoginDays
	if value, err := sysconfig.GetConfig(db, "login_expiry_days", 0); err == nil {
		if parsed, parseErr := strconv.Atoi(value); parseErr == nil && parsed >= 1 && parsed <= maxRememberLoginDays {
			days = parsed
		}
	}
...
func loginTokenTTL(db *gorm.DB, remember bool) time.Duration {
	if !remember {
		return sessionTokenTTL
	}
	return rememberedLoginTTL(db)
}
...
func Register(db *gorm.DB, username string, password string, jwtSecret string) (map[string]interface{}, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		token, err := auth.GenerateTokenWithTTL(user.ID, user.Username, user.Role, user.AuthVersion, jwtSecret, rememberedLoginTTL(tx))
		if err != nil {
			return err
		}
...
func Login(db *gorm.DB, username string, password string, passwordMd5 string, remember bool, jwtSecret string) (map[string]interface{}, error) {
...
func DeleteUser(db *gorm.DB, userID uint) error {
	return db.Transaction(func(tx *gorm.DB) error {
		...
		blocked, err := database.UserDeletionBlocked(tx, userID)
		if err != nil {
			return err
		}
		if blocked {
			return ErrUserHasAppData
		}
		if err := database.CleanupUserAppData(tx, userID); err != nil {
			return err
		}
